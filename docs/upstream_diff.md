...
| Reward weights | `tracking_lin_vel` 1.0, `tracking_ang_vel` 0.75, `feet_air_time` 2.0 with threshold 0.2–0.5 s and no command mask | `tracking_lin_vel` 1.5, `tracking_ang_vel` 1.0, `alive` 0.25, `feet_air_time` 5.0 with threshold 0.1–0.5 s, masked to non-zero commands (env 1.0.5) | Warm-started policies parked in a standing optimum: steps under 0.2 s were penalised and the air-time reward was negligible next to `alive`. Masking stops stepping in place at zero command. |
| `feet_slip` | body xy speed × foot contact (penalises walking itself) | foot xy speed × foot contact (true slip) | Upstream's proxy charges 0.25/step for any body motion during stance, a direct bias toward standing still. |
| Termination | torso upvector z < 0, foot-foot / foot-shin contact, NaN | torso upvector z < 0, pelvis height < 0.35 m, NaN. Self-collision (foot-foot, foot-shin, hand-thigh) is a per-step `collision` penalty (−1.0), not terminal (env 1.0.3) | Spec: self-collision is a penalty, termination is for falls. With upstream's terminal foot-foot contact 93 % of episodes ended that way at 3M samples. Height test: only feet collide with the floor, so a kneeling robot sinks through the ground while still reading "upright". |
