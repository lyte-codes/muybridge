...
| Reward weights | `tracking_lin_vel` 1.0, `tracking_ang_vel` 0.75, `tracking_sigma` 0.25, `feet_air_time` threshold 0.2–0.5 s | `tracking_lin_vel` 1.5, `tracking_ang_vel` 1.0, `tracking_sigma` 0.5, `alive` 0.5, `feet_air_time` threshold 0.1–0.5 s (env 1.0.4) | After standing was learned the tracking reward stayed flat and feet never lifted: at 0.8 m/s error the sigma-0.25 kernel gives ~0.08 reward (no gradient), and steps under 0.2 s were penalised. |
